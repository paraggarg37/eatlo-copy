 ...
 */
angular.module('eatlo.controllers',  ["ngCookies"])
    .controller('mainCtrl',['$scope','$state','$rootScope','$location',function($scope,$state,$rootScope,$location){
...
//            },
    ...
    .controller("menuCtrl", [ "$state","dataService", "$location", "$scope", function ($state, dataService, $location, $scope) {
...
$scope.$parent.showOutlet = true;
            ...
            dataService.getMenu($scope.outlet_id)
...
//                console.log(response);
                ...
                for (var i = 0; i < $scope.all_products.length; i++)
                {
                    ...
                    if ($scope.all_products[i].parent_outlet_product_id === null && $scope.all_products[i].category_id !== 14871 && $scope.all_products[i].category_id !== 14870 && only_for_app_product.indexOf($scope.all_products[i].id) === -1) {
                ...
                if($scope.cart_items.length!==0){
                    ...
                    //alert(JSON.stringify($scope.cart_items));
            ...
            $scope.get_customizations = function (product) {
                ...
                if (product.parent_product) {
                    dataService.getCustomisations($scope.outlet_id,product.outlet_product_id).success(function (response) {
                        if (development_mode) {
                            console.log(response);
                        }
...
//                var checked_count = $("#customization-modal input:checked").length;
                //alert(checked_count);
//                if(checked_count)
                        ...
                        if (product.all_customizations !== undefined)
                        {
                            ...
                            if (product.customization !== undefined){
                                ...
                                for(l = 0 ;l< product.customization.length;l++){
                                    console.log(product.customization[l]);
                                    console.log(product.all_customizations[j].id);
...
//        $route.reload();
    ...
    .controller("outletsCtrl", ["$http", "$location", "$scope", "dataService",function($http, $location, $scope,dataService) {
//        $route.reload();
        $("#share-links").css('display','block');
        $("#share-links-withface").css('display','none');
        /* Scope variables */
        ...
        $scope.api_closest_subarea = function(latitude, longitude) {
            // Call web-service
            dataService.getProxySubAreas(latitude,longitude).success(function(response) {
                if (development_mode) {
                    console.log(response);
                }
                // ... if no results are obtained
                ...
                // ... if results are obtained
                    ...
                    $scope.set_subarea(response.response.docs[0]);
                }
            });
        };
        ...
        $scope.set_subarea = function(subarea) {
            ...
            $(".outlet-container").animate({"opacity": "1"});
...
//            create_alert(2, "Looking for outlets serving " + $scope.selected_subarea.subarea_name + ".", false);
        ...
        $scope.api_nearest_outlets_by_subarea = function() {
            ...
            // Call web-service
            dataService.postdoneOutletsBySubArea(request).success(function(response) {
                if (development_mode) {
                    console.log(response);
                }
            ...
            if ($scope.search_keyword.length >= 2) {
                ...
                $http.get("proxy_company_subareas.php?company_id=" + company_id + "&keyword=" + $scope.search_keyword).success(function(response) {
                    ...
                    // ... if subarea(s) found
                        ...
                        $scope.display_subarea_suggestions = true;
                        $(".outlet-container").animate({"opacity": "0.5"});
                    }
                });
            }
